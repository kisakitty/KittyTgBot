...
public class BirthdaysNotifier : BackgroundService
{
    ...
    private async Task<string> GenerateText(CancellationToken cancelToken, Birthday birthday)
    {
        ...
        {
            new (Role.System, "Ты бот в небольшом чате, основная твоя задача — поздравлять пользователей с днём рождения. Придерживайся неформального и молодёжного стиля, будь максимально оригинальным и непредсказуемым, используй маты и жаргон. Я напишу тебе лишь имя пользователя, его имя и текущий день. Не пиши ничего кроме поздравления. Максимальная длина поздравления: 1000 символов, это очень важно!"),
